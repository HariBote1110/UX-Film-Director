...
- [audio-video-desync-hypothesis.md](audio-video-desync-hypothesis.md) — **追記3: ユーザー実報告条件（`.mov` HEVC 1080p 8570s・隣接proxyなし・プロジェクトfps=24/23.976）を実際にE2E再現して計測。H4（`SHARED_RENDERER_PLAYBACK_PREVIEW_FPS=60`等の60fpsハードコード起因で60/24=2.5倍速）はREJECTED——nativeクロックのframeIndex進行レートはプロジェクトfpsに対し誤差0.1%未満で一致（fps24: 24.02、fps23.976: 23.97）、`ExternalVideoMaxAbsDriftMs`も各run内で変化なし。副産物として「E2Eでprojectfpsを変えるには`useStore.ts`既定値の書き換えだけでは不十分、`src/main.tsx`のE2E専用`initializeProject({fps:60,...})`シードが優先する」というハーネスの落とし穴を発見。／追記2: 指示された「常駐デコーダPTS vs 別HTMLVideoElement」の二重クロック構成は通常のプレビュー再生（videoSourceMode: previewProxy）では成立せず（native-render-frame経路が2回のE2E・計500サンプルで一度も発火せず、videoPresentationSourceは終始external-video-source）。副産物として、GX010052（≈120fps GoPro素材）のproxy生成が実尺・実フレーム数を2.388倍も短く生成する別バグを発見（修正済み: check-proxy/generate-proxyでffprobe検証を追加）。fps不一致説・H3（音声HTMLAudioElement vs 映像ネイティブクロック）もいずれもREJECTED（詳細は本文）**（2026-08-22）
