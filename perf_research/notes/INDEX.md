...
- [audio-video-desync-hypothesis.md](audio-video-desync-hypothesis.md) — **追記2: 指示された「常駐デコーダPTS vs 別HTMLVideoElement」の二重クロック構成は通常のプレビュー再生（videoSourceMode: previewProxy）では成立せず（native-render-frame経路が2回のE2E・計500サンプルで一度も発火せず、videoPresentationSourceは終始external-video-source）。native制御プレーンのframe進行レートは59.92/59.97fpsで壁時計と誤差0.1%未満、2〜3倍速の兆候は未検出（ただしaudio側HTMLVideoElementはDOM非アタッチのため今回未実測、確証には不足）。副産物として、GX010052（≈120fps GoPro素材）のproxy生成が実尺・実フレーム数を2.388倍も短く生成する別バグを発見（タグ上のfpsは元と同一のため気付きにくい）。60fps素材のproxyは正常。fps不一致説・H3（音声HTMLAudioElement vs 映像ネイティブクロック）はいずれもREJECTED（詳細は本文）。VITE_UXFD_RUST_VIDEO_ONLY=1モードは音声が完全に無音になる別の不具合**（2026-08-22）
