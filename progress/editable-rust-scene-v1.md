...
- `spot_light`だけはV1で許可するfilterとし、既存snapshotと同じ
  `rustEffectsForObject`の出力をclipの`effects`へ渡す。SpotLightの光量計算は
  Native WGPU compositorのfragment shaderで行う。
...
- SpotLight以外の有効filterは次段階まで未対応であり、1件でもあるobjectは変換を拒否する。
- resident timelineの音声波形・音声球はV1のProject変換対象外である。PCM window
  の時刻起点は修正済みだが、Native Overlay内でのGPU生成・直接presentは未完である。
