...
- `agent:validate` は2026-08-05〜2026-08-22は `vite-node` 経由で `src/agentProject/agentProject.ts` の `parseAgentProjectSpec` を直接呼んでいたが、2026-08-22のR4-4/R4-5でRust移管し、現在は `rust-core/src/bin/agent_validate.rs`（`parse_agent_project_spec`）を呼ぶ薄いラッパになっている(`scripts/validate-agent-project.mts`)。実行時の唯一の正も `rust-core/src/agent_project.rs` に移った。詳細は `progress/rust-source-of-truth-r4-agent-project.md` を参照。
- 参考用JSON Schemaを `schema/agent-project.schema.json` に置いた。2026-08-22以降は `schemars` 生成物(`npm run codegen:types`)であり、手書き時代にあった `additionalProperties: false` 等の制約は緩くなっているが、実行時の検証には使っていない点は変わらない(ajv等の追加依存を避けた)。エディタ補完・AIの自己検証の補助に留まり、実装との乖離が起きたら `rust-core/src/agent_project.rs` を正とする。
