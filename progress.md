...
- **ユーザー実機確認結果（2026-07-03）**: 位置は正しく追従するが**大幅に遅延する**（ドラッグ中 overlay が遅れてついてくる）。ユーザー判断で当面許容・将来対策のバックログとする。原因候補: resync が `NSWindowDidMove/DidResize` 通知（ドラッグ確定後にまとめて発火しがち）頼みで、`addChildWindow` の OS 既定追従が効いていない可能性。対策候補: ライブドラッグ中の同期（`NSWindowWillMove`＋displayLink での毎フレーム再配置、または child window 追従を OS 既定に任せて手動 resync をリサイズ時のみに限定する切り分け）。
...
- 修正統合後のクリーン起動で、クラッシュの実例だった `葵ちゃん.psd`（2700×3700）を読み込み: **アプリ生存・panic/EPIPE なし・presenter swatch=native-render-frame で描画**を確認（スクリーンショット採取済み）。
- ドラッグ追従はドラッグ途中3点の選択枠計測で経路どおり追従・最終位置一致を確認済み（前エントリ）。版 431a→431b。

### 残課題・次のステップ

- `rust-backend-status` チャンネルの renderer 側購読・UI 通知（sidecar 死亡時のユーザー向け表示）は未実装のバックログ。
- 実マウスでのドラッグ・リサイズ・PSD 表示の最終確認はユーザーに依頼。
...
- 0.1.1-Beta-426c をクリーン起動し CDP で実機検証。動画クリップ（0〜12秒）＋動画なし区間（12秒〜）のタイムラインで以下を確認:
  - 再生→一時停止→動画なし15秒位置へシーク: **残留フレームが消え preview が空表示**（診断は従来どおり noVideoDecodeRequest）。修正前は直前の動画フレームが残留していた。
  - 動画区間（5秒）へ戻る: 動画フレームが正常復帰。再度15秒へ: 再びクリア（往復の遷移が正しく機能）。
  - 動画なし位置で Rectangle を追加: **下層 pixi の赤い矩形が透けて表示**され、overlay クリアが下層コンテンツを遮らないことを確認。
- クリアの毎 tick 再発行はログ出力が無いため実機では未計測（状態機械のガードは vitest で固定済み）。

### 選定理由・判断の根拠

- 検証パターンは「クリア発火」「復帰」「往復」「下層可視」の4点で、状態機械（ok→ガード解除 / noVideoDecodeRequest→1回だけクリア / 一時的失敗→保持）の外形をすべてカバーする。

### 残課題・次のステップ

- Canvas レイヤが UI より上に来る問題（Bug E・child NSWindow 化）はユーザー判断により保留中。今回の修正で「動画のない場面は overlay が透明」になったため体感影響は軽減。
...
- 0.1.1-Beta-425e を `UXFD_OVERLAY_TRACE=1` + CDP で実機検証した。mp4 追加直後・シーク時・再生中のすべての present で以下を確認:
  - `canvas=1920x1080 drawable=1563x879 fit_scale=0.8139 letterbox_offset=(0.17, 0) upload=720x405`
  - `clip[0] transform after: s=(2.1703706, 2.1703706)` → 実効描画サイズ = 720×2.1704 ≈ **1563（drawable 全幅）** / 405×2.1704 ≈ **879（全高）**
  - 2.1703706 = 2.667（decode縮小補正 1920/720）× 0.8139（contain-fit）で理論値と一致
- 途中段階の実測系列も理論と整合: 425c（fit なし）= pane 比 0.46〜0.50、425d（fit のみ・decode縮小未補正）= 0.38、425e = 全幅。
- 最終のピクセル実写確認のみ、検証中にディスプレイがロックされたため未取得（数値検証は完了）。ユーザーの目視確認を依頼する。

### 選定理由・判断の根拠

- 画面ロック中でも検証を継続できるよう、`UXFD_OVERLAY_TRACE` のトレース値（shader 入力直前の transform）を一次証拠として採用した。fit_scale・decode 補正・letterbox の3係数がすべて理論値と一致し、かつ Red→Green の統合テストが shader 入力レベルで固定されているため、視覚結果の乖離が入り込む余地は小さい。
- 教訓: スクリーンショットが全面真っ黒のときは、まずディスプレイのスリープ/ロックを疑う（`caffeinate -u` で起こす）。ロック画面の壁紙はピクセル分散が大きく、分散ベースの領域検出が誤検出するため、ロック解除の確認なしに計測値を信用しない。

### 残課題・次のステップ

- ユーザーによる目視確認（mp4 追加直後のフレームが preview 全体に表示されること）。
- 過去の「Bug B = contentsScale 喪失」修正時に、この decode 縮小比の問題が「1/4 表示」として混同されていた可能性がある。今後 1/4 系の症状を見たら contentsScale / fit / decode 補正の3層を切り分けて確認する。
