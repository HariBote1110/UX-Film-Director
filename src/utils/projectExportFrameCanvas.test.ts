...
const tartanCheck = (patch: Partial<TartanCheckObject> = {}): TartanCheckObject => ({
  id: 'tartan-check-1',
  type: 'tartan_check',
  name: 'タータンチェック',
  layer: 15,
...
const yagasuri = (patch: Partial<YagasuriObject> = {}): YagasuriObject => ({
  id: 'yagasuri-1',
  type: 'yagasuri',
  name: '矢がすり',
  layer: 17,
...
const asanohaPattern = (patch: Partial<AsanohaPatternObject> = {}): AsanohaPatternObject => ({
...
const randomLineEx = (patch: Partial<RandomLineExObject> = {}): RandomLineExObject => ({
  id: 'random-line-ex-1',
  type: 'random_line_ex',
  name: 'ランダムラインEX',
  layer: 21,
...
const hologram = (patch: Partial<HologramObject> = {}): HologramObject => ({
  id: 'hologram-1',
  type: 'hologram',
  name: 'ホログラム',
  layer: 22,
...
const shakingPolygon = (patch: Partial<ShakingPolygonObject> = {}): ShakingPolygonObject => ({
...
const toneCurve = (patch: Partial<ToneCurveObject> = {}): ToneCurveObject => ({
  id: 'tone-curve-1',
  type: 'tone_curve',
  name: '簡易トーンカーブ',
  layer: 25,
...
describe('resolveProjectExportRustFrameSourceContext', () => {
  ...
  it('marks standard particle exports as native-render media instead of legacy canvas work', () => {
    const objects: TimelineObject[] = [particle(), barcode(), puzzlePiece(), colourWheel(), gourd(), gear(), trackBar(), pieChart(), histogram(), toneCurve(), sunburst(), circularArrow(), triangleBracket(), tartanCheck(), houndstooth(), yagasuri(), paperAirplane(), asanohaPattern(), focusLinesPlus(), randomLineEx(), hologram(), protractor(), shakingPolygon(), audio()];
