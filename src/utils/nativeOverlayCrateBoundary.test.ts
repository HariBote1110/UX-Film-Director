...
describe('native overlay napi crate boundary', () => {
  ...
  it('keeps AppKit view code isolated while leaving CAMetalLayer configuration to wgpu', () => {
    ...
    // ではなく、専用 child NSWindow の contentView（container view）の
    // subview として追加される。container view は黒背景の layer-backed view で、
    // overlay（Metal view）はこの container に addSubview: で追加される。
    expect(macosOverlay).toContain('addSubview:');
    expect(macosOverlay).not.toContain('setContentView');
  ...
  it('skips the per-frame upload fence on the live surface present path', () => {
    ...
    expect(livePresentBlock).toContain('prepare_scene_clips_without_upload_fence');
    expect(livePresentBlock).not.toContain('wait_for_submitted_work(&self.core.device, &self.core.queue)');
  });
