...
describe('AviUtlPackV4 feature catalogue', () => {
  ...
  it('selects native UXFD implementations for the effects most useful to voice-video editing', () => {
    ...
    expect(byId.get('93-bezier-orbit-t-plus')?.sourceRelativePaths).toContain('script/93/ベジエ軌道Ｔ+.obj');
    ...
    expect(byId.get('93-border-depth')?.sourceRelativePaths).toContain('script/93/Border.zip');
    ...
    expect(byId.get('93-shattered-sphere')?.implementationTarget).toBe('native-generated-object');
  ...
  it('promotes GetColor, hksy and 93 scripts into the next implementation lane', () => {
    ...
    expect(roadmapIds.slice(0, 43)).toEqual(expect.arrayContaining(priorityIds));
  });
});
