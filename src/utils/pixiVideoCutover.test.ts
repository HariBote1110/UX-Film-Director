...
describe('shouldSkipPixiVideoForSharedRenderer', () => {
  ...
  it('skips every Pixi video branch by default so Pixi cannot own HTMLVideoElement rendering', () => {
    expect(shouldSkipPixiVideoForSharedRenderer()).toBe(true);
  });
...
describe('resolvePixiVideoRenderPath', () => {
  it('requires shared renderer video for ordinary preview clips by default', () => {
    expect(resolvePixiVideoRenderPath()).toBe('sharedRendererOnly');
  });
