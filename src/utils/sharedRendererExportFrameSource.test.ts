...
describe('createSharedRendererExportFrameSource', () => {
  ...
  it('blocks export with native render diagnostics when the Rust native render bridge throws', async () => {
    const canvas = {
      width: 1,
      height: 1,
      dataset: {},
    ...
    const decodedFrame = {
      descriptor: {
        memoryId: '/uxfd-decoded-video-native-render-throw',
        slotIndex: 1,
        generation: 8,
        byteOffset: 256,
        byteLen: 1024,
    ...
    const source = createSharedRendererExportFrameSource({
      ...
      prepareNativeRenderSources: async () => ({
        ok: true,
        activeJobs: [decodeJob('native-render-throw-video')],
        sources: [{
          mediaId: 'video-1',
          slotCount: 2,
          frame: decodedFrame,
      ...
      renderNativeSharedFrame: (async () => {
        calls.push(['renderNativeSharedFrame']);
        throw new Error('native render bridge crashed');
      }) satisfies SharedRendererExportNativeSharedFrameRenderer,
      startViewportPresenter: async () => {
        throw new Error('WebGPU presenter must not start when Rust native render throws.');
      },
    } as unknown as Parameters<typeof createSharedRendererExportFrameSource>[0] & {
      bitmapCaptureEnabled: false;
      renderNativeSharedFrame: unknown;
    ...
    const blocked = await source.renderEncodeFrame?.({
      ...
      height: 4,
      objects: [video()],
      encodeSessionId: 'native-render-throw-session',
    }).catch((error) => error);
  ...
  it('uses native direct encode for mixed native-renderable video and PSD frames when enabled', async () => {
    vi.stubGlobal('window', {
      rustVideoEncoder: {
        nativeDirectEncodeEnabled: true,
        writeNativeEncodeFrame: async () => ({ success: true }),
    ...
    try {
      ...
      const decodedFrame = {
        descriptor: {
          memoryId: '/uxfd-decoded-video-psd-direct',
          slotIndex: 0,
          generation: 9,
          byteOffset: 0,
          byteLen: 1024,
      ...
      const source = createSharedRendererExportFrameSource({
        ...
        prepareNativeRenderSources: (async () => ({
          ok: true,
          activeJobs: [decodeJob('video-psd-direct')],
          sources: [{
            mediaId: 'video-1',
            slotCount: 2,
            frame: decodedFrame,
        ...
        renderNativeSharedFrame: (async () => {
          calls.push(['renderNativeSharedFrame']);
          throw new Error('native shared-frame render must not run when direct encode is enabled for mixed frames.');
        }) satisfies SharedRendererExportNativeSharedFrameRenderer,
        startViewportPresenter: async () => {
          throw new Error('WebGPU presenter must not start for mixed native direct encode.');
        },
      } as unknown as Parameters<typeof createSharedRendererExportFrameSource>[0] & {
        bitmapCaptureEnabled: false;
        prepareNativeRenderSources: unknown;
      ...
      await expect(source.renderEncodeFrame?.({
        ...
        height: 4,
        objects: [image({ id: 'video-placeholder' }), image({ id: 'psd-placeholder' })],
        encodeSessionId: 'video-psd-direct-session',
      })).resolves.toMatchObject({
        timestamp: 133_333,
        nativeEncodeFramePayload: {
          sessionId: 'video-psd-direct-session',
          renderId: 'video-psd-direct-session-frame-8',
          frameIndex: 8,
          timestampUs: 133_333,
          width: 4,
          height: 4,
  ...
  it('falls back before bitmap capture when Rust video upload fails during export', async () => {
    ...
    expect(isSharedRendererExportFrameSourceBlockedError(blocked)).toBe(true);
    expect(blocked.message).toBe('copyReportChecksumMismatch clip=clip-video-2 media=video-2: Shared video frame copy report checksum verification failed.');
    expect(blocked).toMatchObject({
