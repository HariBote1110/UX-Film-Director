...
use uxfd_rust_core::{evaluate_frame, EditableSceneGraph, Project};
...
pub struct Fixture {
    ...
    #[allow(dead_code)]
    pub editable_scene: Option<EditableSceneFixture>,
}

#[derive(Debug, Deserialize)]
pub struct EditableSceneFixture {
    #[allow(dead_code)]
...
pub fn load_fixtures() -> Vec<(String, Fixture)> {
    let dir = fixture_dir();
    ...
    for entry in entries {
        ...
        let fixture: Fixture = serde_json::from_str(&raw).unwrap_or_else(|error| {
            panic!("fixture を parse できない ({}): {error}", path.display())
        });
...
pub fn compare_fixtures<F>(
    fixtures: &[(String, Fixture)],
    mut project_for_fixture: F,
) -> ComparisonSummary
where
    F: FnMut(&Fixture) -> Project,
{
    ...
    for (_source, fixture) in fixtures {
        let project = project_for_fixture(fixture);
        for frame in &fixture.frames {
            compared_frames += 1;
            let actual = evaluate_frame(&project, frame.frame_index);
    ...
    for (path, bucket) in &buckets {
        let (name, frame_index, expected, actual, clip_id) =
            bucket.example.as_ref().expect("bucket には必ず例が入る");
        let delta = if bucket.max_delta > 0.0 {
            format!("{:.6}", bucket.max_delta)
        } else {
            "-（構造差）".to_string()
        };
        let bucket_detail = format!(
        ...
        match known.get(path) {
            None => {
                ...
                failures.push_str(&bucket_detail);
            }
    ...
    ComparisonSummary {
        compared_frames,
        compared_fixtures: fixtures.len(),
        rounding,
        failures,
        detail,
    }
...
impl ComparisonSummary {
    pub fn assert_matches_known_differences(&self, comparison_name: &str) {
        if !self.failures.is_empty() {
            panic!(
                "{comparison_name}の差分が既知のベースラインと違う。\n\
                 fixture = {} 件 / 比較フレーム = {}\n\
                 丸め差(許容 {TOLERANCE}) = {} 件\n\n{}\n\
                 既知の差分は {} で管理している。",
                self.compared_fixtures,
    ...
    #[allow(dead_code)]
    pub fn assert_non_zero_frames(&self) {
        assert!(
            self.compared_frames > 0,
            "比較フレーム数が 0。fixture の読み込みまたは比較経路が空になっている"
        );
    }
    ...
    pub fn print(&self) {
        println!(
            "比較フレーム = {} / 丸め差 = {} 件\n既知の差分（R2 で解消する）:\n{}",
            self.compared_frames, self.rounding, self.detail
        );
    }
}
