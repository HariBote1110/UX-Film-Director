...
if (cargo.error) {
  console.error(
    `[build-native-overlay-addon] failed to start "${cargoBin} ${cargoArgs.join(' ')}" (cwd: ${repoRoot}): ${cargo.error.message}`
  )
  if (cargo.error.code === 'ENOENT') {
    console.error(buildCargoNotFoundHint('build-native-overlay-addon'))
  }
  process.exit(1)
}
