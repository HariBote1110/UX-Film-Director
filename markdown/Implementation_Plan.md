...
23. `P2` E2Eで代表素材に効果を載せ、preview/exportの一致を検証（境界E2Eと実Electron生成効果exportは確認済み。Audio waveform RはRust native renderer内で直接フレーム/共有メモリフレームの画素一致と指定色画素の可視性を確認済み。Rust backendではAudio waveform R+標準パーティクル混在の共有メモリ合成も確認済み。Audio waveform RのPCM取得はsource frameへ追従済み。実Electron成果物の画素比較は継続）
