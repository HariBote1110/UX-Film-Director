...
1. **クロスオブジェクト配線マッピング**（`editableRustScene.ts` /
   ...
   **→ 調査済み・保留（2026-09-13）**: Rust到達時点で解決入力が失われているため、
   関数単位の移管は不可と判断。「編集用object graphをRustへ渡し評価用Project/mediaを
   Rustで構築する」再設計が前提（`progress/cross-object-wiring-to-rust.md`）。
