...
- `src/utils/sharedRendererExportFrameSource.ts`
- `nativeRenderUnsupportedMedia` を投げる2経路へfallback可否を渡し、Rust/native render必須時はlegacy canvas fallbackを禁止するようにした。
- Rust必須exportで未対応mediaをPixi/legacy captureに退避させず、未対応範囲を明示的に止める。
- `package.json` / `package-lock.json`
- バージョンを `0.1.1-Beta-215t` に更新した。

## 確認
- `npm test -- sharedRendererExportFrameSource` を実行し、Redでunsupported native media時もlegacy fallbackが許可される失敗を確認した。
- `npm test -- sharedRendererExportFrameSource` を再実行し、41件成功を確認した。
- `npm test -- sharedRendererExportFrameSource projectExportFrameRenderer exportDiagnosticsLog exportProgress sharedRendererNativeMediaSupport` を実行し、76件成功を確認した。
- `npx tsc --noEmit 2>&1 | rg "(src/utils/sharedRendererExportFrameSource\\.ts|src/utils/sharedRendererExportFrameSource\\.test\\.ts|src/utils/projectExportFrameRenderer\\.ts|src/utils/exportDiagnosticsLog\\.ts|src/utils/exportProgress\\.ts|src/utils/sharedRendererNativeMediaSupport\\.ts)"` を実行し、対象ファイルに型エラーが出ないことを確認。

## 117. Phase5: native renderer未接続ではRust必須時のlegacy fallbackを禁止
- `src/utils/sharedRendererExportFrameSource.test.ts`
- 動画encode frameとencode-only frameでRust native renderer bridgeが未接続の場合、`fallbackToLegacyCanvas=false` / `legacyCanvasFallbackAllowed=false` になる契約へ更新した。
- `src/utils/sharedRendererExportFrameSource.ts`
- `nativeRenderUnavailable` を投げる経路で `legacyCanvasFallbackAllowed=false` を渡すようにした。
- native renderer未接続をPixi/legacy captureへ退避させず、Rust必須exportとして明示的に停止する。
- `package.json` / `package-lock.json`
- バージョンを `0.1.1-Beta-215u` に更新した。

## 確認
- `npm test -- sharedRendererExportFrameSource` を実行し、Redでnative renderer未接続時もlegacy fallbackが許可される失敗を確認した。
- `npm test -- sharedRendererExportFrameSource` を再実行し、41件成功を確認した。
- `npm test -- sharedRendererExportFrameSource projectExportFrameRenderer exportDiagnosticsLog exportProgress` を実行し、72件成功を確認した。
- `npx tsc --noEmit 2>&1 | rg "(src/utils/sharedRendererExportFrameSource\\.ts|src/utils/sharedRendererExportFrameSource\\.test\\.ts|src/utils/projectExportFrameRenderer\\.ts|src/utils/exportDiagnosticsLog\\.ts|src/utils/exportProgress\\.ts)"` を実行し、対象ファイルに型エラーが出ないことを確認。
...
## 119. Phase5: video upload/ownership失敗ではlegacy fallbackを禁止
- `src/utils/sharedRendererExportFrameSource.test.ts`
- `videoUploadFailed` と `videoOwnershipUnavailable` のexport blockで、`fallbackToLegacyCanvas=false` / `legacyCanvasFallbackAllowed=false` になる契約を追加した。
- stale decode responseでも `videoUploadFailed` としてfallback不可になることを明示した。
- `src/utils/sharedRendererExportFrameSource.ts`
- Rust video upload失敗とvideo ownership未成立の `SharedRendererExportFrameSourceBlockedError` へ `legacyCanvasFallbackAllowed=false` を渡すようにした。
- Rust decode/upload/ownership cutoverが成立していない動画exportを、Pixi/legacy captureへ退避させない。
- `package.json` / `package-lock.json`
- バージョンを `0.1.1-Beta-215w` に更新した。

## 確認
- `npm test -- sharedRendererExportFrameSource` を実行し、Redでvideo upload/ownership失敗時もlegacy fallbackが許可される失敗を確認した。
- `npm test -- sharedRendererExportFrameSource` を再実行し、41件成功を確認した。
- `npm test -- sharedRendererExportFrameSource projectExportFrameRenderer exportDiagnosticsLog exportProgress` を実行し、72件成功を確認した。
- `npx tsc --noEmit 2>&1 | rg "(src/utils/sharedRendererExportFrameSource\\.ts|src/utils/sharedRendererExportFrameSource\\.test\\.ts|src/utils/projectExportFrameRenderer\\.ts|src/utils/exportDiagnosticsLog\\.ts|src/utils/exportProgress\\.ts)"` を実行し、対象ファイルに型エラーが出ないことを確認。

## 120. Phase5: native render source準備失敗ではRust必須時のlegacy fallbackを禁止
- `src/utils/sharedRendererExportFrameSource.test.ts`
- `prepareNativeRenderSources` が `staleDecodeResponse` で失敗した場合、`nativeRenderFailed` blockで `fallbackToLegacyCanvas=false` / `legacyCanvasFallbackAllowed=false` になる契約を追加した。
- `src/utils/sharedRendererExportFrameSource.ts`
- source準備一般失敗で `nativeRenderFailed` に丸める場合も、Rust/native render必須時はlegacy canvas fallbackを禁止するようにした。
- 互換モードでnative renderが任意の場合だけ、従来のfallback許可を維持する。
- `package.json` / `package-lock.json`
- バージョンを `0.1.1-Beta-215x` に更新した。

## 確認
- `npm test -- sharedRendererExportFrameSource` を実行し、Redでsource準備失敗時もlegacy fallbackが許可される失敗を確認した。
- `npm test -- sharedRendererExportFrameSource` を再実行し、42件成功を確認した。
- `npm test -- sharedRendererExportFrameSource projectExportFrameRenderer exportDiagnosticsLog exportProgress` を実行し、73件成功を確認した。
- `npx tsc --noEmit 2>&1 | rg "(src/utils/sharedRendererExportFrameSource\\.ts|src/utils/sharedRendererExportFrameSource\\.test\\.ts|src/utils/projectExportFrameRenderer\\.ts|src/utils/exportDiagnosticsLog\\.ts|src/utils/exportProgress\\.ts)"` を実行し、対象ファイルに型エラーが出ないことを確認。

## 121. Phase5: blocked errorのlegacy fallbackを明示opt-inに変更
- `src/utils/sharedRendererExportFrameSource.test.ts`
- `SharedRendererExportFrameSourceBlockedError` が明示指定なしでは `fallbackToLegacyCanvas=false` / `legacyCanvasFallbackAllowed=false` になる契約を追加した。
- `src/utils/sharedRendererExportFrameSource.ts`
- blocked errorの `legacyCanvasFallbackAllowed` 既定値をfalseに変更した。
- 互換fallbackとして残すsurface gate blockは、呼び出し側で `true` を明示するようにした。
- 新しいRust export block reasonを追加した時に、指定漏れでPixi/legacy captureへ戻る状態を避ける。
- `package.json` / `package-lock.json`
- バージョンを `0.1.1-Beta-215y` に更新した。

## 確認
- `npm test -- sharedRendererExportFrameSource` を実行し、Redでblocked errorのdefault fallbackが許可される失敗を確認した。
- `npm test -- sharedRendererExportFrameSource` を再実行し、43件成功を確認した。
...
## 131. Phase5: shared renderer output blocked診断の表示ラベルを追加
- `src/components/ExportProgressModal.test.ts`
- `sharedRendererOutputUnavailable` のRust frame source blocked診断がraw reasonではなく、日本語/英語の読みやすいラベルで表示される契約を追加した。
- `src/components/ExportProgressModal.tsx`
- blocked reason formatterに `sharedRendererOutputUnavailable` を追加した。
- 日本語では `shared renderer実出力なし`、英語では `shared renderer output unavailable` と表示する。
- `package.json` / `package-lock.json`
- バージョンを `0.1.1-Beta-216i` に更新した。
...
## 133. Phase5: 実出力必須時のvideo presentation失敗をblocked診断に変更
- `src/utils/sharedRendererPreviewPresenterController.test.ts`
- `requireSharedRendererOutput` 有効時にRust decoded video frame upload後のvideo frame scene presentationが `webGpuDrawUnavailable` で失敗した場合、dataset statusが `blocked` になる契約を追加した。
- テスト用 `fakeDevice` に `exposeCreateSampler` / `exposeCreateBindGroup` を追加し、WebGPU video draw API欠落を再現できるようにした。
- `src/utils/sharedRendererPreviewPresenterController.ts`
- video frame scene presentation失敗時のdiagnostics statusを、通常previewでは `fallback` のまま、実出力必須時だけ `blocked` に切り替えるようにした。
- `package.json` / `package-lock.json`
- バージョンを `0.1.1-Beta-216k` に更新した。

## 確認
- `npm test -- sharedRendererPreviewPresenterController` を実行し、Redで `webGpuDrawUnavailable` がまだ `fallback` と出る失敗を確認した。
- `npm test -- sharedRendererPreviewPresenterController` を再実行し、36件成功を確認した。
...
## 136. Phase5: 実出力必須時のnative render presentation失敗をblocked診断に変更
- `src/utils/sharedRendererPreviewPresenterController.test.ts`
- `requireSharedRendererOutput` 有効時にnative render frame presentationが `nativeRenderTextureViewUnavailable` で失敗した場合、dataset statusが `blocked` になる契約を追加した。
- テスト用 `fakeDevice` に `createTexture` 差し替えを追加し、upload成功後にtexture viewだけ欠落する状態を再現できるようにした。
- `src/utils/sharedRendererPreviewPresenterController.ts`
- native render frame presentation失敗時のdiagnostics statusを、通常previewでは `fallback` のまま、実出力必須時だけ `blocked` に切り替えるようにした。
- `package.json` / `package-lock.json`
- バージョンを `0.1.1-Beta-216n` に更新した。

## 確認
- `npm test -- sharedRendererPreviewPresenterController` を実行し、Redで `nativeRenderTextureViewUnavailable` がまだ `fallback` と出る失敗を確認した。
- `npm test -- sharedRendererPreviewPresenterController` を再実行し、37件成功を確認した。
...
- `src/utils/sharedRendererExportFrameSource.ts`
- `throwNativeRenderSourceReleaseFailed` と `throwNativeRenderOutputReleaseFailed` が `SharedRendererExportFrameSourceBlockedError` を投げる時、legacy canvas fallbackを常に禁止するようにした。
- Rust decoded sourceやnative render outputの所有権解放失敗を、Pixi/legacy captureで成功扱いにしない。
- `package.json` / `package-lock.json`
- バージョンを `0.1.1-Beta-215r` に更新した。

## 確認
- `npm test -- sharedRendererExportFrameSource` を実行し、Redでrelease失敗時もlegacy fallbackが許可される失敗を確認した。
- `npm test -- sharedRendererExportFrameSource` を再実行し、41件成功を確認した。
- `npm test -- sharedRendererExportFrameSource projectExportFrameRenderer exportDiagnosticsLog exportProgress` を実行し、72件成功を確認した。
- `npx tsc --noEmit 2>&1 | rg "(src/utils/sharedRendererExportFrameSource\\.ts|src/utils/sharedRendererExportFrameSource\\.test\\.ts|src/utils/projectExportFrameRenderer\\.ts|src/utils/exportDiagnosticsLog\\.ts|src/utils/exportProgress\\.ts)"` を実行し、対象ファイルに型エラーが出ないことを確認。
