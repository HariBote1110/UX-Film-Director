...
## 追記2（2026-08-09）: 或窓式琴葉茜(細)——コーパス中の最重量ファイル

対象: `或窓式琴葉茜立ち絵素材(細).psd`（**162MB・922ノード**・8bit、デコード総量1.63GB）。
ファイルサイズ・ノード数ともコーパス最大。

| 計測 | 結果 |
|---|---|
| メタデータ同等性 | **完全一致**（922ノード、クラッシュ無し） |
| rust メタのみパース | **0.286ms** |
| ag-psd レイヤーのみ readPsd（Node） | 1794ms |
| rust 全量デコード N=8 | 370ms（対ag-psd 4.8倍速） |

- 922ノードでもメタのみは0.3ms未満。インポート経路の非依存性がさらに強く裏付けられた。
- ag-psd はこのファイルで約1.8秒（Node直）。in-app Worker ならさらに悪化するはずで、
  現行経路の実害が最も大きいクラスのファイル。
