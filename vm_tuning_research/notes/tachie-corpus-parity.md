...
## 追記（2026-08-09）: ユーザー提供の最大立ち絵での追加検証

対象: `琴葉茜立ち絵ver0.7.psd`（50.8MB、89ノード、8bit、可視リーフ57/89）。
特徴: ノード数は少ないがレイヤーがほぼ全面サイズで、**全量デコードは1.57GB**
（葵ちゃん.psdの4.6倍）と、これまでのコーパスで最大のピクセル負荷。

| 計測 | 結果 |
|---|---|
| メタデータ同等性（ag-psd vs rust） | **完全一致**（89ノード、クラッシュ無し） |
| rust メタのみパース | **0.094ms** |
| ag-psd レイヤーのみ readPsd（Node） | 1259ms |
| ag-psd フル readPsd（Node） | 1390ms |
| rust 全量デコード serial | 761ms |
| rust 全量デコード N=8 | 286ms（対ag-psd 4.4倍速） |
| rust 可視のみ N=8 | 222ms（可視バイト比86%のため削減は小さい） |

- インポート経路（メタのみ）は 0.094ms で、ピクセル量に完全に非依存であることを
  最大負荷ファイルでも確認。in-app の ag-psd Worker なら実測傾向から約2.5秒級に
  なっていたはずのファイルが、path B′ では数十ms オーダーに収まる見込み。
- 可視限定デコードの効果はファイル依存（このファイルは初期可視が86%を占め効果薄）。
  遅延デコードを本実装へ入れる場合も「可視率が高いファイルでは効かない」前提で設計する。
